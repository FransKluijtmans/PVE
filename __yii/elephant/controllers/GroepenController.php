...
class GroepenController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Groepen']))
		{
			$model->attributes=$_POST['Groepen'];
			$model->userAangemaakt= Yii::app()->session['personeelsnummer'];
			$model->datumAangemaakt= date("Y-m-d H:i:s");
			if($model->save())
				Yii::app()->user->setFlash('success', '<strong>Goed gedaan!</strong> Groep '.$model->naam.' is succesvol aangemaakt.');
	...
	{
		/*$dataProvider=new CActiveDataProvider('Groepen');
	...
	{
		$model=new Groepen('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Groepen']))
			$model->attributes=$_GET['Groepen'];
	...
	{
		$model=Groepen::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='groepen-form')
		{
