...
{	
	...
	public function rules()
	{
		...
		return array(
			array('omschrijving, datum, datumAangemaakt, secties_id, userAangemaakt', 'required', 'on' => 'create'),
			array('omschrijving, datum, datumAangepast, secties_id, userAangepast', 'required', 'on' => 'update'),
			...
			array('id, omschrijving, datum, datumAangemaakt, datumAangepast, secties_id, userAangemaakt, userAangepast, naam', 'safe', 'on'=>'search'),
		);
	}
	...
	{	
		if($this->contents){
			if (AgendaHasContent::model()->exists('agenda_id=:agenda',
				array( ':agenda'=>$this->id )
			)) {
		        $criteria=new CDbCriteria;
	    		$criteria->condition='agenda_id=:agenda';
				$criteria->params=array(':agenda'=>$this->id);
				AgendaHasContent::model()->deleteAll($criteria);
			}

			if(is_array($this->contents)){
				foreach ($this->contents as $contentId) {
					$agenda = new AgendaHasContent;
					$agenda->agenda_id = $this->id;
					$agenda->content_id = $contentId;
					if (!$agenda->save()) 
						//Leden::model()->deleteAll('personeelsnummer= "'.$this->personeelsnummer.'"');
						print_r($agenda->errors);
				}
			}else{
				$agenda = new AgendaHasContent;
				$agenda->agenda_id = $this->id;
				$agenda->content_id = $this->contents;
				if (!$agenda->save()) 
					//Leden::model()->deleteAll('personeelsnummer= "'.$this->personeelsnummer.'"');
					print_r($agenda->errors);
			}
