...
class AuthModule extends CWebModule
{
	...
	public function beforeControllerAction($controller, $action)
	{
		if (parent::beforeControllerAction($controller, $action))
		{
			...
			{
				//if ($user->isAdmin)
				if ($user->isAdmin || Yii::app()->user->checkAccess('auth.*') || Yii::app()->user->checkAccess('admin')) //22/02/2013 - added check on if user hase the operation auth.* to build alle authorisations.
					return true;
			}
			else
